...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `[ALARM] Incoming: device=${device_id} type=${event_type} ts=${timestamp}`
    );
    ...
    const [customer] = await db
      ...
      .where(
        or(
          eq(customers.deviceIdHub, device_id),
          eq(customers.deviceIdMobile, device_id)
        )
      )
      .limit(1);
    ...
    const alertSource =
      customer?.deviceIdHub === device_id ? "hub" : "mobile";
    ...
    if (event_type === "heartbeat") {
      ...
      const status =
        batteryLevel !== null && batteryLevel < 20 ? "low_battery" : "online";
    ...
    const [newCase] = await db
      ...
      .values({
        ...
        notes: customer
          ? undefined
          : `Unknown device: ${device_id}. Raw: ${raw}`,
      })
      .returning();
